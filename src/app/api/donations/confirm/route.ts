 ...
 */
export async function POST(request: NextRequest): Promise<NextResponse<ApiResponse<DonationResponse>>> {
  try {
    await connectDB();
    ...
    if (paymentIntent.charges?.data?.[0]?.payment_method_details) {
      ...
      donation.paymentMethod = paymentMethodDetails.type || 'card';
    }

    // Extract country from Stripe (for anonymized analytics)
    if (paymentIntent.charges?.data?.[0]?.billing_details?.address?.country) {
      donation.country = paymentIntent.charges.data[0].billing_details.address.country;
    }
