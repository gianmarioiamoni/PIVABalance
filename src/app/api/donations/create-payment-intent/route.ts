 ...
 */
export async function POST(request: NextRequest): Promise<NextResponse<ApiResponse<StripePaymentIntent>>> {
  try {
    await connectDB();
    ...
    const paymentIntent = await stripe.paymentIntents.create({
      ...
      description: `PIVABalance Donation${validatedData.message ? ` - ${validatedData.message}` : ''}`,
      receipt_email: validatedData.isAnonymous ? undefined : validatedData.donorEmail,
    });
    ...
    const donationData = {
      ...
      donorEmail: validatedData.isAnonymous ? undefined : validatedData.donorEmail,
    ...
    return NextResponse.json(
      { success: true, data: response, message: 'Payment intent created successfully' },
      { status: 201 }
    );
 ...
 */
export async function GET(): Promise<NextResponse<ApiResponse<{ publishableKey: string }>>> {
  try {
