...
export const InvoiceList = ({
    ...
    return (
        <div className="mt-8 flow-root">
            ...
            {/* Mobile Card View - Visible only on screens smaller than 640px */}
            <div className="block sm:hidden space-y-4">
                {invoices.map((invoice, index) => (
                    <div key={invoice.id || `invoice-mobile-${index}`} className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
                        {/* Header Row */}
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-2 text-sm text-gray-500">
                                <User className="h-4 w-4" />
                                <span className="truncate" title={invoice.clientName}>{invoice.clientName}</span>
                            </div>

                            {/* Title */}
                            <div className="text-sm text-gray-500">
                                <span className="font-medium">Titolo:</span> {invoice.title}
                            </div>

                            {/* VAT */}
                            {invoice.vat && (
                                <div className="text-sm text-gray-500">
                                    <span className="font-medium">IVA:</span> {invoice.vat.vatRate}%
                                </div>
                            )}
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-2 flex-1">
                                    <input
                                        ...
                                        disabled={updatingPayment === invoice.id}
                                        className="block w-full min-w-0 rounded-md border-gray-300 shadow-sm text-gray-900 focus:border-indigo-500 focus:ring-indigo-500 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                                        aria-label={`Data pagamento per fattura ${invoice.number}`}
                                    />
                                    {updatingPayment === invoice.id && (
                                        <LoadingSpinner size="sm" />
